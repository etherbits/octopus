...
const QueueList = () => {
  ...
  return (
    <div className="flex flex-col w-80 p-4 bg-neutral-950 rounded-xl shadow-xl max-h-full overflow-hidden">
      ...
      <ul className="flex flex-col gap-4 overflow-auto flex-grow-[1]">
        {tracks.length < 1 ? (
          <span className="text-neutral-400">no tracks in queue</span>
        ) : (
          tracks.map((track) => (
            <li key={track.id}>
              <button
